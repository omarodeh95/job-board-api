class Api::V1::MyJobPostsController < ApplicationController
    ...
    def new
        ...
        render json: {data: @job_post.as_json(only: [:title, :description])}
    end
    ...
    def create
        ...
        @job_post = @user.job_posts.new(job_post_params)

        if @job_post.save
            render json: {message: "Job post created successfully"}, status: :ok
        else
